...
const Step2ChooseDateTime = ({ onNextStep, onPreviousStep }) => {
    const [selectedDate, setSelectedDate] = useState('');
    const [selectedTime, setSelectedTime] = useState('');

    // Generate dummy time slots for a day
    const timeSlots = [];
    for (let hour = 8; hour <= 18; hour++) { // From 8 AM to 6 PM
        ...
        if (hour < 18) { // Add 30-minute slots up to 5:30 PM
            timeSlots.push(`${hour}:30 ${hour < 12 ? 'AM' : 'PM'}`);
        }
    }
                        ...
                        {timeSlots.map((slot) => (
                            <button
                                ...
                                className={`p-3 rounded-lg border text-sm font-inter transition-all duration-200 ease-in-out
                                    ${selectedTime === slot ? 'bg-[#cc6500] text-white shadow-md' : 'bg-gray-100 text-gray-700 border-gray-200 hover:bg-gray-200'}`}
                            >
                                {slot}
                            </button>
                        ))}
